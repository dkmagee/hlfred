...
#
# Nice improvements by Ewald Zietsman <ewald.zietsman@gmail.com>
#
# $Id: sexcatalog.py,v 1.7 2007/09/04 09:44:12 llg Exp $
#
# ======================================================================
#
...
version: 0.1.7 - last modified: 2007-09-04
...
class SExtractorfile:
    ...
    _SE_keys = \
             {"NUMBER"         : {"comment": "Running object number",
              ...
              "FLUX_APER"      : {"comment": "Flux in aperture",
                                  "infunc": float,
                                  "format": "%12g",
                                  "unit": "count"},
                                      
              "FLUXERR_APER"   : {"comment": "Error of flux in aperture",
                                  "infunc": float,
                                  "format": "%12g",
                                  "unit": "count"},
                                      
              "MAG_APER"       : {"comment": "Magnitude in aperture",
                                  "infunc": float,
                                  "format": "%12g",
                                  "unit": "magnitude"},
                                      
              "MAGERR_APER"    : {"comment": "Magnitude error in aperture",
                                  "infunc": float,
                                  "format": "%12g",
                                  "unit": "magnitude"},
    ...
    def readline(self):
        ...
        # __values now contains an entire line (without newline) in the catalog (after the header)
        __values = __ll.split()



# ----------   modifications for multiple apertures starts here -------------- #
...
# if multiple apertures are specified, these are returned as lists else they're like before
        ...
        for i in range(len(self.ez_vals)):
            if self.ez_vals[i] < maxval:
                temp = __values[self.ez_vals[i]:self.ez_vals[i+1]]
                    
                if len(temp) == 1:
                    # if there is only 1 entry, add it as a number. This will
                    # make the program work as before for single apertures
                    self.__newvals.append(temp[0])
                else:
                    self.__newvals.append(temp)
            else:
                temp = __values[self.ez_vals[i]:]
        ...
        for i in self._keys:
            # if it is a single value or string
            if isinstance(self._outdict[i],(str,int,float)):
                self._outdict[i] = (
                    SExtractorfile._SE_keys[i]["infunc"](self._outdict[i]))
            else:
                # if it is a list of strings
                temp = self._outdict[i]
                self._outdict[i] = [SExtractorfile._SE_keys[i]["infunc"](item) for item in temp]
                
# ----------   modifications for multiple apertures ends here -------------- #        
